...
| 2 | Wire `PluginManager` / `WorkflowRegistry` / `McpToolRegistry` as host-owned services on `container.extensions` | 🔄 Partial — `WorkflowRegistry` wired (host-owned, `add_path()` lands per-extension workflows at stage 3); `container.workflow_registry` field added; silent `except: continue` replaced with logged WARNING. `PluginManager` + `McpToolRegistry` deferred (orthogonal to extension lifecycle — model-provider plugins + MCP tools are steps 3 + 7). |
